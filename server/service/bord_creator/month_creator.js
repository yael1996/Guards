class MonthCreator {
  ...
  build_month(month, year) {
    ...
    this.dates_by_type();
    this.fill_month_shifts();
  }

  fill_month_shifts() {
    this.fill_spacial_dates_shifts(
      this.special_dates,
      this.bord.special_dates_shift_description,
      special_dates
    );

    this.fill_spacial_dates_shifts(
      this.special_days,
      this.bord.special_days_shift_description,
      special_days
    );

    this.fill_spacial_dates_shifts(
      this.regular_days,
      this.bord.regular_days_shift_description,
      regular_days
    );
  ...
  dates_by_type(month, year) {
    for (var i = 1; i <= this.num_total_days_in_month(); i++) {
      ...
      if (this.bord.special_dates.any(d => d.getTime() === currDate.getTime()))
